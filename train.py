...
from modeling.encoder_decoder import SpecRuleEncoderDecoderConfig, SpecRuleEncoderDecoderModel
...
def main():
    ...
    send_example_telemetry("run_clm", data_args)
    ...
    input_dataset = raw_datasets.map(collator)
    ...
    def tokenize_function(examples, spec_size):
        ...
        with CaptureLogger(tok_logger) as cl:
            ...
            encoder_features = \
                encoder_tokenizer(list(it.chain.from_iterable(s['annotated_sentences'] for s in examples['spec'])),
            ...
            decoder_features['labels'] = torch.cat(
                (decoder_features['input_ids'], torch.ones((decoder_features['input_ids'].size()[0], 1))*torch.Tensor(decoder_tokenizer.encode(decoder_tokenizer.eos_token))), dim=-1).long()

        return {
            **encoder_features,
            **{
              f"decoder_{k}":v for k, v in decoder_features.items()
            }
        }
    ...
    # TODO A lot of this tokenization functions will go away
    ...
    trainer = Trainer(
        ...
        tokenizer=encoder_tokenizer,    # TODO deal with this when it is better understood
    ...
    kwargs = {"finetuned_from": "bert-base-uncased", "tasks": "text-generation"} # TODO fix the hardcoded model name
    if data_args.dataset_name is not None:
